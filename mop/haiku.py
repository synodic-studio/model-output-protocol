"""Reference Haiku evaluator — wraps Anthropic Haiku via pydantic-ai.

`build_haiku_evaluator(rules)` returns an `Evaluator`-shaped callable
suitable for passing into `MOP(evaluator=...)`. It batches every active
rule into a single prompt and asks Haiku for one structured verdict
per submission.
...
API key precedence:
  MOP_ANTHROPIC_API_KEY   preferred — kept out of subprocess env so it
...
def build_haiku_evaluator(
    *, rules: list[Rule]
    ...
    _agent = None  # lazy — defer API-key check until first call
