...
class TestHeightPenalty:
    ...
    def test_linear_region(self):
        ...
        text = "\n".join(["short"] * 18)
        assert height_penalty(text, budget=32) == 1.0
    ...
    def test_cliff_region(self):
        ...
        # total = 48
        text = "\n".join(["short"] * 34)
        assert height_penalty(text, budget=32) == 48.0


class TestStructurePenalty:
    def test_empty(self):
        assert structure_penalty("") == 0.0

    def test_few_prose_lines_free(self):
        """Up to PROSE_FREE_LINES prose lines incur no penalty."""
        text = "\n".join(["prose line"] * PROSE_FREE_LINES)
        assert structure_penalty(text) == 0.0

    def test_excess_prose_lines(self):
        """Prose lines beyond free allowance incur 1 point each."""
        text = "\n".join(["prose line"] * (PROSE_FREE_LINES + 3))
        assert structure_penalty(text) == 3.0
    ...
    def test_headers_exempt(self):
        text = "# Header\n## Subheader\nprose line"
        assert structure_penalty(text) == 0.0  # 1 prose, within free allowance
    ...
    def test_blank_lines_exempt(self):
        text = "prose\n\nmore prose\n\n\nlast prose"
        assert structure_penalty(text) == 0.0  # 3 prose, within free allowance
...
class TestFormatScore:
    ...
    def test_short_message(self):
        ...
        result = format_score("The build passed. Tests are green.")
        assert result["format_score"] == 0.0

    def test_returns_expected_keys(self):
        result = format_score("hello world")
